...
const FIVESIM_PRICING = {
    ...
    'ca': { country: 'Canada', flag: '🇨🇦', services: { telegram: 1.20, whatsapp: 1.20, discord: 1.20 } },
...
async function getAvailableNumbers(country, service) {
    try {
        ...
        console.log(`Retrieved ${mockNumbers.length} available numbers for ${country}/${service}`);
        return mockNumbers;
    } catch (error) {
...
async function buyNumber(phoneNumber, country, service, durationDays = 0.014) {
    try {
        ...
        // Convert to naira (1 USD ≈ ₦1,200)
        const priceNGN = priceUSD * 1200;

        // Apply markup (100% profit margin)
        ...
        await saveNumber(
            ...
            service,
            purchaseId,
            sellingPrice / 100 // Convert to kobo
        );
        ...
        console.log(`Number purchased: ${phoneNumber} (${purchaseId})`);
        ...
        return {
            ...
            message: `Number ${phoneNumber} purchased successfully`
        };
    } catch (error) {
